...
static int f_modscol(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    ...
    // G_effective
    //ZW's edit starts
    Fvir = mymgF(ET*ET0, myh, myenv, Rth, OM, OCB, (data->theory), IC, (data->mymodel));
    //ZW's edit ends
     ...
     for(int i = 0; i< (*xxyyzz).count; i++ ){
      ...
      // potential energy (always the same):
       ...
       if(model==1){
         ...
         wphi = prefac * mymgF(ai, myy, myyenv, Rthp, omegacb, omegacb,extpars, myscolparams[0],model)*mydelt;
